const PUBLIC_APPLICATION_CODES = new Set([
  'AMBIENT_CONFIG_DETECTED', 'BODY_TOO_LARGE', 'CONFLICT', 'CONTEXT_BUDGET_EXCEEDED', 'CONTEXT_SOURCE_UNAVAILABLE', 'CREATE_FAILED',
  ...
  'INVALID_INPUT', 'INVALID_INTENT', 'INVALID_MESSAGE', 'INVALID_MUTATION_ID', 'INVALID_NAME', 'INVALID_ORG',
  ...
  'RUNTIME_AUTH_UNAVAILABLE', 'RUNTIME_START_FAILED', 'RUNTIME_TURN_TIMEOUT', 'RUNTIME_UNSUPPORTED', 'SANDBOX_UNAVAILABLE', 'STALE_PROPOSAL', 'UNAUTHENTICATED',
...
export function closedRuntimeErrorCode(error: unknown, harness?: string, method?: string): string {
  ...
  if (/auth|credential/i.test(category)) return 'RUNTIME_AUTH_UNAVAILABLE';
